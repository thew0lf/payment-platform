          ...
          <div className="grid md:grid-cols-3 gap-6 mb-10">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
                ...
                <h3 className="font-semibold text-white mb-2">Feature {i}</h3>
...
function ProductSelectionPreview({ config, onContinue, products }: ProductSelectionPreviewProps) {
  ...
  if (layout === 'comparison') {
    return (
      <div className="min-h-[600px] bg-gray-50 dark:bg-gray-900 py-12 px-6">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-8 text-gray-900 dark:text-gray-100">Choose Your Plan</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {displayProducts.slice(0, 3).map((product, index) => (
              ...
              >
                {index === 1 && (
                  <div className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-4">
                    Most Popular
                  </div>
                )}
  ...
  return (
    <div className="min-h-[600px] bg-gray-50 dark:bg-gray-900 py-12 px-6">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold text-center mb-8 text-gray-900 dark:text-gray-100">Select Products</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {displayProducts.map((product) => (
            ...
            >
              <div className="aspect-square bg-gray-100 dark:bg-gray-700 rounded-xl mb-4 flex items-center justify-center text-4xl">
                {product.image}
              </div>
              <h3 className="font-semibold text-sm mb-1 text-gray-900 dark:text-gray-100">{product.name}</h3>
              {display.showDescription && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Product description goes here</p>
              )}
              {display.showPrices && (
                <p className="font-bold text-indigo-600 dark:text-indigo-400">${product.price}</p>
              )}
            </div>
          ))}
        </div>
...
function CheckoutPreview({ config, onComplete, products = [] }: CheckoutPreviewProps) {
  ...
  const inputClass =
    'w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm';
      ...
      <div className={`max-w-4xl mx-auto ${layout === 'two-column' ? 'grid md:grid-cols-5 gap-8' : ''}`}>
        ...
        <div className={layout === 'two-column' ? 'md:col-span-3' : ''}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
            ...
            {shippingEnabled && (
              <div className="mb-6">
                <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300 mb-3">Shipping Address</h3>
                <div className="space-y-3">
            ...
            {/* Payment */}
            <div className="mb-6">
              <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300 mb-3">Payment</h3>
              <div className="space-y-3">
            ...
            {(payment.allowCoupons as boolean) && (
              <div className="mb-6">
                <div className="flex gap-2">
                  ...
                  <button className="px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                    Apply
                  </button>
                </div>
              </div>
        ...
        {layout === 'two-column' && (payment.showOrderSummary as boolean) !== false && (() => {
          ...
          return (
            <div className="md:col-span-2">
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
                <h3 className="font-semibold mb-4 text-gray-900 dark:text-gray-100">Order Summary</h3>
                {displayProducts.map((product, index) => (
                  <div key={index} className="flex gap-4 pb-4 border-b border-gray-200 dark:border-gray-700 mb-4 last:mb-0">
                    ...
                    <p className="font-medium text-sm text-gray-900 dark:text-gray-100">${product.price.toFixed(2)}</p>
                  </div>
                ))}
...
function ThankYouPreview({ config }: ThankYouPreviewProps) {
  ...
  return (
    <div className="min-h-[600px] bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-900 dark:to-emerald-900 flex items-center justify-center py-12 px-6">
      <div className="text-center max-w-md">
        <div className="w-20 h-20 rounded-full bg-green-500 flex items-center justify-center mx-auto mb-6">
          <CheckCircle className="w-10 h-10 text-white" />
        </div>
        <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-4">{headline}</h2>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          ...
          <p className="text-xl font-mono font-bold text-gray-900 dark:text-gray-100">A-000-000-001</p>
        </div>
      </div>
    </div>
  );
...
export default function TemplatePreviewPage() {
  ...
  if (error || !template) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Template Not Found</h2>
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col">
      ...
      <div className="h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-semibold text-gray-900 dark:text-gray-100">{template.name}</h1>
        ...
        {stages.length > 1 && (
          <div className="flex md:hidden justify-center gap-2 mt-4">
            <button
              ...
              className="px-4 py-2 bg-white dark:bg-gray-800 rounded-lg font-medium text-gray-700 dark:text-gray-300 disabled:opacity-50"
            >
              Back
            </button>
