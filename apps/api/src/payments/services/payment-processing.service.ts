...
export class PaymentProcessingService {
  ...
  private emitEvent(type: PaymentEventType, companyId: string, providerId: string, data: Record<string, unknown>): void { const event: PaymentEvent = { id: `${Date.now()}-${Math.random().toString(36).substring(2, 8)}`, type, providerId, companyId, transactionId: (data as { transactionId?: string }).transactionId, data, timestamp: new Date() }; this.eventEmitter.emit(type, event); this.eventEmitter.emit('payment.*', event); }
}
